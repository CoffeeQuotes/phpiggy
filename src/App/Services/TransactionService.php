...
class TransactionService
{
    ...
    public function getUserTransactions(int $length, int $offset)
    {
        ...
        $transactionsCount = $this->db->query("SELECT COUNT(*) FROM transactions WHERE user_id = :user_id AND description LIKE :description", $params)->count();
    ...
    public function update(array $formData, int $id)
    {
        ...
        $this->db->query(
            "UPDATE transactions
            SET description = :description, amount = :amount, date = :date WHERE id = :id AND user_id = :user_id",
            [
                'description' => $formData['description'],
                'amount' => $formData['amount'],
                'date' => $formattedDate,
